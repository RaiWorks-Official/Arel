...
function UI:CreateWindow(Config)
    local Window = {
        ...
        User = Config.User or {},
        Music_Player = Config.Music_Player or false,
        Tabs = {},
        AllElements = {},
        CurrentTab = {},
        TabOrder = {},
    ...
    if Window.Music_Player then
        local Sound_Service = game:GetService("SoundService")
        local Music = {
            folder = "Music_Library",
            tracks = {},
            index = 0,
            playing = false,
            volume = 0.5,
        ...
        Music.sound = Instance.new("Sound")
        Music.sound.Name = "Music_Player"
        Music.sound.Volume = Music.volume
        Music.sound.Parent = Sound_Service

        local function Music_Refresh()
            Music.tracks = {}
            if typeof(listfiles) == "function" and typeof(isfolder) == "function" then
                if typeof(makefolder) == "function" and not isfolder(Music.folder) then
                    makefolder(Music.folder)
                end
                local files = listfiles(Music.folder)
                if files then
                    for _, path in ipairs(files) do
                        local name = string.match(tostring(path), "[^/\\]+$") or tostring(path)
                        table.insert(Music.tracks, { path = path, name = (string.gsub(name, "%.[%w]+$", "")) })
                    end
                end
            end
        end
        ...
        local Music_Play, Music_Toggle, Music_Next, Music_Previous

        Music_Play = function(i)
            local count = #Music.tracks
            if count == 0 then return end

            if i < 1 then
                i = Music.looped and count or 1
            elseif i > count then
                i = Music.looped and 1 or count
            end

            local track = Music.tracks[i]
            if not track then return end

            if typeof(getcustomasset) == "function" then
                local id = getcustomasset(track.path)
                if id then
                    Music.sound.SoundId = id
                end
            end
        ...
        Music_Toggle = function()
            if #Music.tracks == 0 then return end

            if Music.index == 0 then
                Music_Play(Music.shuffle and math.random(1, #Music.tracks) or 1)
                return
            end
            ...
            if Music.playing then
                Music.sound:Pause()
                Music.playing = false
                MusicToggle.Image = GetIcon("play")
            else
                Music.sound:Resume()
                Music.playing = true
                MusicToggle.Image = GetIcon("pause")
        ...
        Music_Next = function()
            if #Music.tracks == 0 then return end
            local n
            if Music.shuffle and #Music.tracks > 1 then
                repeat
                    n = math.random(1, #Music.tracks)
                until n ~= Music.index
            else
                n = Music.index + 1
            end
        ...
        Music_Previous = function()
            if #Music.tracks == 0 then return end
            Music_Play(Music.index - 1)
        end
        ...
        MusicToggle.MouseButton1Click:Connect(Music_Toggle)
        ...
        for _, t in ipairs(Music.tracks) do
            table.insert(TrackOptions, t.name)
        end

        PersonalTab:Dropdown({
            Title = "Track",
            Desc = "Select a track to play",
            Option = TrackOptions,
